...
void AuraProcessor::initialiseSmoothers (double sampleRate)
{
    ...
    monoAmount.reset (sampleRate, 0.010);
...
void AuraProcessor::process (juce::AudioBuffer<float>& buffer, const AuraParameters& p) noexcept
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        ...
        left = micCharacter.processRelativeSample (0, left, p.sourceMicMode, p.targetMicMode,
                                                   correctionValue, targetValue, badFrequencyValue,
        ...
        right = micCharacter.processRelativeSample (juce::jmin (1, channels - 1), right,
                                                    p.sourceMicMode, p.targetMicMode, correctionValue,
                                                    targetValue, badFrequencyValue, airProtectionValue,
                                                    bodyProtectionValue, p.hardwareSafeMode);
        ...
        left = preamp.processSample (0, left, p.preampMode, preampDriveValue);
        right = preamp.processSample (juce::jmin (1, channels - 1), right, p.preampMode, preampDriveValue);
        ...
        SummingGlue::processStereo (left, right, juce::jlimit (0.0f, 1.0f, summingMix + consoleDensityValue),
        ...
        widthProcessor.processStereo (left, right, widthValue, false);
