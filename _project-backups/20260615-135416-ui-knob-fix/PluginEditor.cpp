    ...
    : AudioProcessorEditor (&p), processorRef (p)
{
    ...
    setResizeLimits (1120, 720, 1920, 1150);
    setSize (1400, 860);
    inputKnob.setHeroStyle (true);
    outputKnob.setHeroStyle (true);
    ...
    for (auto* label : { &logoTitle, &logoSubtitle, &auraPercent, &presetCard, &latencyLabel, &oversamplingLabel, &monitorLabel })
        addAndMakeVisible (*label);
    ...
    for (auto* c : { static_cast<juce::Component*> (&inputRms), static_cast<juce::Component*> (&outputRms),
                     ...
                     static_cast<juce::Component*> (&eqDisplay), static_cast<juce::Component*> (&auraRing),
                     static_cast<juce::Component*> (&trackButtons), static_cast<juce::Component*> (&compModeButtons),
                     static_cast<juce::Component*> (&qualityBar) })
        addAndMakeVisible (*c);
...
void StadiumAuraAudioProcessorEditor::layoutKnobRow (juce::Rectangle<int>& area, std::initializer_list<juce::Slider*> knobs)
{
    ...
    for (auto* knob : knobs)
        knob->setBounds (area.removeFromLeft (w).reduced (2));
}
...
void StadiumAuraAudioProcessorEditor::layoutKnobGrid (juce::Rectangle<int>& area, int columns, std::initializer_list<juce::Slider*> knobs)
{
    ...
    for (auto* knob : knobs)
    {
        ...
        knob->setBounds (juce::Rectangle<int> (area.getX() + col * cellW, area.getY() + row * cellH, cellW, cellH).reduced (2));
        ++index;
    }
}
...
void StadiumAuraAudioProcessorEditor::resized()
{
    ...
    auto topBar = bounds.removeFromTop (78);
    auto footerBar = bounds.removeFromBottom (58);
    auto meterBar = bounds.removeFromBottom (88);
    auto eqRow = bounds.removeFromBottom (108);
    auto main = bounds.reduced (0, 4);

    logoTitle.setBounds (topBar.withSizeKeepingCentre (420, 34).removeFromTop (34));
    logoSubtitle.setBounds (topBar.withSizeKeepingCentre (520, 18).removeFromBottom (18));
    auto nav = topBar.removeFromBottom (28).reduced (40, 0);
    monitorLabel.setBounds (topBar.removeFromBottom (16).reduced (20, 0));
    const auto navW = nav.getWidth() / static_cast<int> (routing.size());
    for (auto& button : routing)
        button.setBounds (nav.removeFromLeft (navW).reduced (1));

    auto left = main.removeFromLeft (juce::roundToInt (main.getWidth() * 0.24f));
    auto inMeter = main.removeFromLeft (52);
    auto center = main.removeFromLeft (juce::roundToInt (main.getWidth() * 0.52f));
    ...
    auto leftArea = leftPanel.getBounds().reduced (8, 28);
    inputKnob.setBounds (leftArea.removeFromTop (juce::roundToInt (leftArea.getHeight() * 0.28f)).reduced (8, 2));
    auto toneRow = leftArea.removeFromTop (leftArea.getHeight() / 4);
    layoutKnobRow (toneRow, { &bodyKnob, &presenceKnob, &airKnob });
    auto harmRows = leftArea.removeFromTop (leftArea.getHeight() * 55 / 100);
    layoutKnobGrid (harmRows, 3, { &tubeDriveKnob, &saturation, &tubeBias, &transformer, &summing, &glue });
    auto matchBlock = leftArea.removeFromTop (leftArea.getHeight());
    sourceMic.setBounds (matchBlock.removeFromTop (20));
    matchBlock.removeFromTop (2);
    targetMic.setBounds (matchBlock.removeFromTop (20));
    matchBlock.removeFromTop (2);
    layoutKnobRow (matchBlock, { &correction, &targetAmount });
    badFreq.setBounds (matchBlock.reduced (2));
    auto bottomLeft = leftArea;
    hardwareSafe.setBounds (bottomLeft.removeFromBottom (20).reduced (4, 0));
    analyzeSource.setBounds (bottomLeft.removeFromBottom (20).reduced (4, 0));
    preampMode.setBounds (bottomLeft.removeFromBottom (20));
    preampDrive.setBounds (bottomLeft.reduced (bottomLeft.getWidth() / 4, 2));

    auto hero = heroPanel.getBounds().reduced (8, 28);
    tubeChamber.setBounds (hero.removeFromTop (juce::roundToInt (hero.getHeight() * 0.34f)).reduced (4, 0));
    auto auraBlock = hero;
    sweetZone.setBounds (auraBlock.removeFromBottom (14));
    auto sweetRow = auraBlock.removeFromBottom (14);
    ...
    auraPercent.setBounds (auraBlock.removeFromBottom (20));
    auraRing.setBounds (auraBlock.withSizeKeepingCentre (juce::jmin (auraBlock.getWidth(), auraBlock.getHeight() - 10),
                                                          juce::jmin (auraBlock.getWidth(), auraBlock.getHeight() - 10)));
    aura.setBounds (auraBlock.reduced (auraBlock.getWidth() / 7, 0));
    trackButtons.setBounds (hero.removeFromTop (24).reduced (40, 0));
    tubeType.setBounds (hero.removeFromTop (22).reduced (30, 0));
    consoleMode.setBounds (hero.removeFromBottom (22).reduced (20, 0));

    auto rightArea = rightPanel.getBounds().reduced (8, 28);
    compModeButtons.setBounds (rightArea.removeFromTop (24));
    rightArea.removeFromTop (4);
    auto vuArea = rightArea.removeFromRight (juce::roundToInt (rightArea.getWidth() * 0.42f));
    vuMode.setBounds (vuArea.removeFromBottom (20).reduced (2));
    compressorEnable.setBounds (vuArea.removeFromBottom (20).reduced (2));
    vuMeter.setBounds (vuArea.reduced (2));
    auto dynRow = rightArea.removeFromTop (rightArea.getHeight() / 2);
    layoutKnobRow (dynRow, { &compAmount, &compMakeup, &attack, &release });
    layoutKnobRow (rightArea, { &threshold, &ratio, &bleed });
    auto outRow = rightArea.removeFromBottom (rightArea.getHeight() / 2);
    auto mixRow = outRow.removeFromTop (outRow.getHeight() / 2);
    layoutKnobRow (mixRow, { &mix, &width });
    outputKnob.setBounds (outRow.removeFromLeft (outRow.getWidth() * 2 / 3).reduced (4, 2));
    ceiling.setBounds (outRow.reduced (4, 2));

    eqDisplay.setBounds (eqPanel.getBounds().reduced (10, 26));

    auto meters = meterBar.reduced (2, 4);
    grMeter.setBounds (meters.removeFromLeft (meters.getWidth() / 3).reduced (4, 6));
    auto util = meters.removeFromLeft (meters.getWidth() / 2);
    limiter.setBounds (util.removeFromLeft (72).reduced (2, 16));
    ...
    limMeter.setBounds (meters.reduced (4, 6));

    auto footer = footerBar.reduced (2, 4);
    helpBtn.setBounds (footer.removeFromLeft (28).reduced (2, 14));
    presetPrev.setBounds (footer.removeFromLeft (28).reduced (2, 14));
    presetCard.setBounds (footer.removeFromLeft (180).reduced (4, 12));
    presets.setBounds (footer.removeFromLeft (juce::jmax (120, footer.getWidth() / 3)).reduced (4, 12));
    presetNext.setBounds (footer.removeFromLeft (28).reduced (2, 14));
    saveBtn.setBounds (footer.removeFromLeft (52).reduced (2, 14));
    favoriteBtn.setBounds (footer.removeFromLeft (28).reduced (2, 14));
    abA.setBounds (footer.removeFromLeft (24).reduced (2, 14));
    abB.setBounds (footer.removeFromLeft (24).reduced (2, 14));
    settingsBtn.setBounds (footer.removeFromRight (28).reduced (2, 14));
    oversamplingLabel.setBounds (footer.removeFromRight (120).reduced (2, 14));
    latencyLabel.setBounds (footer.removeFromRight (120).reduced (2, 14));
}
