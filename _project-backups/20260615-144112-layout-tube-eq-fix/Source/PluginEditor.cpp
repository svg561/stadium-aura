...
void StadiumAuraAudioProcessorEditor::resized()
{
    ...
    auto footerBar = bounds.removeFromBottom (76);
    auto meterBar = bounds.removeFromBottom (106);
    auto eqRow = bounds.removeFromBottom (118);
    auto main = bounds.reduced (0, 4);

    auto logo = topBar.removeFromLeft (270);
    logoTitle.setBounds (logo.removeFromTop (30));
    logoSubtitle.setBounds (logo.removeFromTop (16));
    ...
    undoBtn.setBounds (topBar.removeFromLeft (42).reduced (2, 12));
    auto nav = topBar.removeFromLeft (juce::jmax (360, topBar.getWidth() - 96)).reduced (2, 4);
    ...
    settingsBtn.setBounds (topBar.removeFromLeft (28).reduced (2, 12));
    helpBtn.setBounds (topBar.removeFromLeft (28).reduced (2, 12));
    monitorLabel.setBounds (topBar.reduced (2, 12));

    auto inMeter = main.removeFromLeft (58);
    auto outMeter = main.removeFromRight (58);
    const auto leftW = juce::roundToInt (main.getWidth() * 0.28f);
    ...
    sourceMic.setBounds (leftArea.removeFromTop (22));
    leftArea.removeFromTop (3);
    targetMic.setBounds (leftArea.removeFromTop (22));
    leftArea.removeFromTop (8);
    auto matchKnobs = leftArea.removeFromTop (158);
    layoutKnobGrid (matchKnobs, 2, { &correction, &targetAmount, &badFreq, &bodyKnob, &airKnob });
    auto micButtons = leftArea.removeFromTop (24);
    analyzeSource.setBounds (micButtons.reduced (2));
    leftArea.removeFromTop (6);
    hardwareSafe.setBounds (leftArea.removeFromTop (24).reduced (2));
    leftArea.removeFromTop (8);
    auto charRow = leftArea.removeFromTop (114);
    layoutKnobRow (charRow, { &presenceKnob, &mix });
    preampMode.setBounds (leftArea.removeFromTop (22));
    leftArea.removeFromTop (4);
    preampDrive.setBounds (leftArea.removeFromTop (78).withSizeKeepingCentre (78, 78));

    auto hero = heroPanel.getBounds().reduced (10, 30);
    auto tubeColumn = hero.removeFromRight (132);
    tubeDriveKnob.setBounds (tubeColumn.removeFromTop (100).withSizeKeepingCentre (82, 82));
    tubeBias.setBounds (tubeColumn.removeFromTop (86).withSizeKeepingCentre (72, 72));
    tubeType.setBounds (tubeColumn.removeFromTop (24).reduced (4, 0));
    tubeChamber.setBounds (tubeColumn.reduced (4, 8));
    auto auraBlock = hero.removeFromTop (juce::jmin (hero.getHeight() - 62, hero.getWidth() - 44));
    aura.setBounds (auraBlock.withSizeKeepingCentre (juce::jmin (auraBlock.getWidth(), auraBlock.getHeight()),
                                                     juce::jmin (auraBlock.getWidth(), auraBlock.getHeight())).reduced (8));
    auto sweetRow = hero.removeFromTop (18).reduced (70, 0);
    ...
    trackButtons.setBounds (hero.removeFromBottom (26).reduced (70, 0));

    auto rightArea = rightPanel.getBounds().reduced (10, 30);
    compModeButtons.setBounds (rightArea.removeFromTop (26));
    rightArea.removeFromTop (6);
    auto vuArea = rightArea.removeFromRight (juce::roundToInt (rightArea.getWidth() * 0.40f));
    vuMode.setBounds (vuArea.removeFromBottom (22).reduced (2));
    compressorEnable.setBounds (vuArea.removeFromBottom (22).reduced (2));
    vuMeter.setBounds (vuArea.reduced (2));
    auto dynTop = rightArea.removeFromTop (92);
    layoutKnobRow (dynTop, { &compAmount, &compMakeup, &attack, &release });
    auto dynBottom = rightArea.removeFromTop (82);
    layoutKnobRow (dynBottom, { &threshold, &ratio, &bleed });
    rightArea.removeFromTop (8);
    auto outputTop = rightArea.removeFromTop (82);
    layoutKnobRow (outputTop, { &compMakeup, &bleed, &ceiling });
    outputKnob.setBounds (rightArea.withSizeKeepingCentre (116, 116));

    auto lower = eqPanel.getBounds().reduced (10, 26);
    auto lowerLeft = lower.removeFromLeft (juce::roundToInt (lower.getWidth() * 0.24f));
    layoutKnobRow (lowerLeft, { &saturation, &transformer });
    auto lowerMid = lower.removeFromLeft (juce::roundToInt (lower.getWidth() * 0.22f));
    consoleMode.setBounds (lowerMid.removeFromTop (24).reduced (4, 0));
    trackButtons.setBounds (lowerMid.removeFromTop (26).reduced (4, 2));
    layoutKnobRow (lowerMid, { &summing, &glue });
    auto limiterBox = lower.removeFromRight (190);
    limiter.setBounds (limiterBox.removeFromTop (28).reduced (8, 2));
    ceiling.setBounds (limiterBox.removeFromTop (76).withSizeKeepingCentre (70, 70));
    limMeter.setBounds (limiterBox.reduced (6, 2));
    eqDisplay.setBounds (lower.reduced (6, 0));
    if (eqDisplay.isExpanded())
        eqDisplay.setBounds (getLocalBounds().reduced (140, 120));
    ...
    width.setBounds (meters.removeFromLeft (96).reduced (4, 8));
    outputKnob.setBounds (meters.reduced (4, 8).withSizeKeepingCentre (86, 86));

    auto footer = footerBar.reduced (2, 4);
    inputFader.setBounds (footer.removeFromLeft (130).reduced (4, 2));
    inputLrMeter.setBounds (footer.removeFromLeft (82).reduced (4, 2));
    bypass.setBounds (footer.removeFromLeft (82).reduced (6, 16));
    mono.setBounds (footer.removeFromLeft (72).reduced (6, 16));
    dim.setBounds (footer.removeFromLeft (72).reduced (6, 16));
    presets.setBounds (footer.removeFromLeft (juce::jmax (220, footer.getWidth() / 3)).reduced (8, 18));
    undoBtn.setBounds (footer.removeFromLeft (54).reduced (6, 18));
    redoBtn.setBounds (footer.removeFromLeft (54).reduced (6, 18));
    outputLrMeter.setBounds (footer.removeFromRight (82).reduced (4, 2));
    outputFader.setBounds (footer.removeFromRight (130).reduced (4, 2));
    oversamplingLabel.setBounds (footer.removeFromRight (120).reduced (2, 18));
